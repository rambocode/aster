// Input bridge adapted from umputun/agterm and thdxg/macterm (MIT).
...
extension GhosttySurfaceView {
  ...
  private func modifiers(_ event: NSEvent) -> ghostty_input_mods_e {
    ...
    // AppKit's public modifier flags collapse left/right keys. libghostty accepts side
    // bits, so preserve them from the stable device-dependent flag positions.
    let deviceFlags = flags.rawValue
    let leftShift: UInt = 0x02
    let rightShift: UInt = 0x04
    let leftControl: UInt = 0x01
    let rightControl: UInt = 0x2000
    let leftOption: UInt = 0x20
    let rightOption: UInt = 0x40
    let leftCommand: UInt = 0x08
    let rightCommand: UInt = 0x10
    if deviceFlags & rightShift != 0, deviceFlags & leftShift == 0 {
      raw |= GHOSTTY_MODS_SHIFT_RIGHT.rawValue
    }
    if deviceFlags & rightControl != 0, deviceFlags & leftControl == 0 {
      raw |= GHOSTTY_MODS_CTRL_RIGHT.rawValue
    }
    if deviceFlags & rightOption != 0, deviceFlags & leftOption == 0 {
      raw |= GHOSTTY_MODS_ALT_RIGHT.rawValue
    }
    if deviceFlags & rightCommand != 0, deviceFlags & leftCommand == 0 {
  ...
  private func unshiftedCodepoint(_ event: NSEvent) -> UInt32 {
    // `characters(byApplyingModifiers:)` asserts for flagsChanged events; a bare
    // modifier has no printable codepoint, so zero is the correct C API value.
    guard
