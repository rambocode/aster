...
/// Direction used by host applications to extend a native terminal selection from the caret.
public enum TerminalSelectionDirection: Sendable {
    case left
    case right
    case up
    case down
...
extension TerminalView {
    ...
    func selectedColumnsRange(row: Int, cols: Int) -> Range<Int>? {
        ...
        if selection.isRectangular {
            ...
            guard (firstRow...lastRow).contains(row) else { return nil }
            let lowerBound = max(0, min(startCol, endCol))
            let upperBound = min(cols, max(startCol, endCol))
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        for row in firstRow...lastRow {
            ...
            let lineOrigin = CGPoint(
                x: 0,
                y: frame.height - lineOffset + viewportContentTranslationY
            )
    ...
    func updateCursorPosition()
    {
        ...
        let lineOrigin = CGPoint(
            x: 0,
            y: frame.height - offset + viewportContentTranslationY
        )
        #endif
    ...
    /// Jumps to the oldest row in the normal scrollback buffer.
    public func scrollToTop() {
        guard !terminal.isDisplayBufferAlternate else { return }
        scrollTo(row: 0)
    }

    /// Jumps to the newest normal-buffer viewport and clears smooth/overscroll translation.
    public func scrollToBottom() {
        ...
        let buffer = terminal.displayBuffer
        scrollTo(row: max(0, buffer.lines.count - buffer.rows))
    }
    ...
    /// Whether the active selection uses a fixed column interval on every covered row.
    public var isSelectionRectangular: Bool {
        selection.active && selection.isRectangular
    }

    /// Extends the native selection from the current terminal caret by one character or row.
    @discardableResult
    public func extendSelection(
        _ direction: TerminalSelectionDirection,
        rectangular: Bool = false
    ) -> Bool {
        let buffer = terminal.displayBuffer
        let cursor = Position(col: buffer.x, row: buffer.yBase + buffer.y)
        let changed = selection.extendFromCursor(
            cursor,
            direction: direction,
