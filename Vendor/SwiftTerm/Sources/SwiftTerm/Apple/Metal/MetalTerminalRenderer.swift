...
struct CacheSignature: Hashable {
    ...
    let yDisp: Int
    let viewportTranslationY: Double
    let rows: Int
    let cols: Int
    let fontName: String
    let fontSize: Double
...
final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    private func buildDrawDataPass(scale: CGFloat) -> DrawData {
        ...
        let signature = CacheSignature(scale: Double(scale),
                                       ...
                                       yDisp: visibleDisp,
                                       viewportTranslationY: Double(terminalView.viewportContentTranslationY),
                                       rows: buffer.rows,
                                       cols: buffer.cols,
    ...
    private func visibleRowRange(buffer: Buffer,
        ...
        let extraBottomRow = terminalView.viewportContentTranslationY > 0 ? 1 : 0
        let lastRow = min(
            buffer.lines.count - 1,
            buffer.yDisp + buffer.rows - 1 + extraBottomRow
        )
        if firstRow > lastRow {
            return nil
        }
    ...
    private func buildRowDrawData(row: Int,
        ...
        let lineOrigin = CGPoint(
            x: 0,
            y: terminalView.bounds.height - lineOffset
                + terminalView.viewportContentTranslationY
        )
        ...
        for shaped in shapedSegments {
            ...
            for run in shaped.runs {
                ...
                let runCellCount = run.shaperRun.cellCount
                let startColumn = shaped.segment.column + (processedCells * shaped.segment.columnWidth)
                let endColumn = startColumn + (runCellCount * shaped.segment.columnWidth)
            ...
            for run in shaped.runs {
                ...
                for glyphRun in run.shaperRun.glyphRuns {
                    ...
                    for i in 0..<glyphRun.glyphs.count {
                        ...
                        let glyphColumn = startColumn
                            + (glyphRun.cellOffsets[i] * shaped.segment.columnWidth)
                ...
                if let rawStyle = runAttributes[.underlineStyle] as? Int,
                   rawStyle != 0 {
                    ...
                    for cellOffset in 0..<(runCellCount * shaped.segment.columnWidth) {
                        let glyphColumn = startColumn + cellOffset
    ...
    private func buildShapedSegments(_ segments: [ViewLineSegment], terminalView: TerminalView) -> [ShapedSegment] {
        ...
        for segment in segments {
            ...
            segment.attributedString.enumerateAttributes(in: NSRange(location: 0, length: segment.attributedString.length),
                ...
                let ligatureLevel = attributes[.ligature] as? Int ?? 0
                guard let shaped = shaperCache.shape(
                    text: text,
                    font: runFont as CTFont,
                    ligatureLevel: ligatureLevel) else {
                    return
                }
    ...
    private struct ShaperGlyphRun {
        ...
        /// Visual-order offsets measured in terminal characters, not glyph count.
        let cellOffsets: [Int]
    }
    ...
    private final class ShaperCache {
        ...
        func shape(text: String, font: CTFont, ligatureLevel: Int) -> ShaperRun? {
            guard !text.isEmpty else {
                return nil
            }
            ...
            let attributedString = NSAttributedString(
                string: text,
                attributes: [.font: font, .ligature: ligatureLevel])
            ...
            var utf16CellOffsets: [Int] = []
            var cellCount = 0
            for character in text {
                utf16CellOffsets.append(
                    contentsOf: repeatElement(cellCount, count: String(character).utf16.count))
                cellCount += 1
            }
            var processedCells = 0
            for run in runs {
                ...
                var visualOffset = processedCells
                for (fallback, index) in stringIndices.enumerated() {
                    ...
                    cellOffsets.append(visualOffset)
                    previousCell = cell
                }
                ...
                processedCells += Set(utf16CellOffsets[lower..<upper]).count
                glyphRuns.append(ShaperGlyphRun(
                    font: runFont,
                    glyphs: glyphs,
    ...
    private static func candidateBundles() -> [Bundle] {
        ...
        // `AsterTerminal_SwiftTerm.bundle`. Its only two candidates are
        ...
        // the build machine's absolute `.build/.../AsterTerminal_SwiftTerm.bundle`
