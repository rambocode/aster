...
/// Where normal-buffer scrolling may stop after passing the newest terminal row.
public enum ScrollPastLastLineMode: Sendable {
    case disabled
...
/// Where normal-buffer scrolling may stop before the oldest terminal row.
public enum ScrollPastFirstLineMode: Sendable {
    case disabled
    case sameAsLastLine
...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// When enabled, any user input sent to the child process drops the active selection first.
    /// This includes text, navigation keys, Backspace, Tab, paste, and IME commits.
    public var clearSelectionOnTyping = true

    /// Signed visual translation applied after row-based `yDisp` positioning. Positive values
    /// move terminal rows upward; negative values reveal blank space above the oldest row.
    public internal(set) var viewportContentTranslationY: CGFloat = 0 {
        didSet {
            guard oldValue != viewportContentTranslationY else { return }
    ...
    public var scrollPastLastLineMode: ScrollPastLastLineMode = .disabled
    public var scrollPastFirstLineMode: ScrollPastFirstLineMode = .disabled
    ...
    func calculateMouseHit (at point: CGPoint) -> (grid: Position, pixels: Position)
    {
        ...
        // Rendering translates rows independently from yDisp while smooth/overscroll is active;
    ...
    /// retained even if Option or Shift changes mid-drag, preventing orphan releases or a TUI
    /// button that remains logically pressed.
    private enum MouseGestureOwner: Equatable {
        case nativeSelection
        case terminalReporting
    }

    private var activeMouseGestureOwner: MouseGestureOwner?
    private var activeNativeSelectionIsRectangular = false

    open override func mouseDown(with event: NSEvent) {
        activeMouseGestureOwner = nil
        activeNativeSelectionIsRectangular = false
        ...
        {
            activeMouseGestureOwner = .terminalReporting
            sharedMouseEvent(with: event)
            return
        }
        activeMouseGestureOwner = .nativeSelection
        activeNativeSelectionIsRectangular = event.modifierFlags.contains(.option)
        
        selection.preparePointerSelection(rectangular: event.modifierFlags.contains(.option))
        
        switch event.clickCount {
        case 1:
            if selection.active && event.modifierFlags.contains(.shift) {
                selection.shiftExtend(bufferPosition: Position(col: hit.col, row: hit.row))
            } else {
                selection.beginPointerSelection(
                    at: Position(col: hit.col, row: hit.row),
                    rectangular: event.modifierFlags.contains(.option)
                )
            }
        case 2:
            let displayBuffer = terminal.displayBuffer
    ...
    open override func mouseUp(with event: NSEvent) {
        let gestureOwner = activeMouseGestureOwner
        activeMouseGestureOwner = nil
        activeNativeSelectionIsRectangular = false
        stopSelectionAutoScrollTimer()
        autoScrollDelta = 0
        lastSelectionDragPoint = nil
        if gestureOwner == .terminalReporting {
    ...
    open override func mouseDragged(with event: NSEvent) {
        ...
        guard let gestureOwner = activeMouseGestureOwner else { return }
        if gestureOwner == .terminalReporting {
        ...
        selection.preparePointerSelection(rectangular: activeNativeSelectionIsRectangular)
        if selection.active {
            selection.dragExtend(bufferPosition: Position(col: hit.col, row: hit.row))
        } else {
    ...
    /// Leftover precise delta used only when smooth scrolling is disabled.
    ...
    /// Moves the normal-buffer viewport by a raw wheel/trackpad delta. A continuous point-space
    /// position drives both fractional smooth scrolling and bounded overscroll; row-based yDisp
    /// remains valid, so terminal buffer indexing and alternate-screen behavior stay unchanged.
    public func scrollViewport(deltaY: CGFloat, precise: Bool, gestureEnded: Bool = false) {
        guard deltaY != 0 || gestureEnded else { return }
        guard let cellHeight = cellDimension?.height, cellHeight > 0 else { return }
        guard !terminal.isDisplayBufferAlternate else {
            viewportContentTranslationY = 0
            return
        }

        let buffer = terminal.displayBuffer
        let maxRow = max(0, buffer.lines.count - buffer.rows)
        let baseMaximum = CGFloat(maxRow) * cellHeight
        let topLimit = topOverscrollLimit(cellHeight: cellHeight)
        let bottomLimit = bottomOverscrollLimit(cellHeight: cellHeight, maximumRow: maxRow)
        var position = CGFloat(buffer.yDisp) * cellHeight + viewportContentTranslationY

        if precise && smoothScrollEnabled {
            position -= deltaY * scrollSensitivity
        } else if deltaY != 0 {
            scrollAccumulator += deltaY * scrollSensitivity
            let lines = precise
                ? Int(scrollAccumulator / cellHeight)
                : (Int((deltaY * scrollSensitivity).rounded()) == 0
                    ? (deltaY > 0 ? 1 : -1)
                    : Int((deltaY * scrollSensitivity).rounded()))
            if precise {
                scrollAccumulator -= CGFloat(lines) * cellHeight
            } else {
                scrollAccumulator = 0
            }
            position -= CGFloat(lines) * cellHeight
        }

        position = min(baseMaximum + bottomLimit, max(-topLimit, position))
        if gestureEnded {
            position = (position / cellHeight).rounded() * cellHeight
            scrollAccumulator = 0
        }
        applyVirtualScrollPosition(position, cellHeight: cellHeight, maximumRow: maxRow)
    }

    /// Reapplies the current virtual position after host preferences change. More restrictive
    /// overscroll modes clamp existing blank space immediately, and disabling smooth scrolling
    /// removes a fractional row instead of leaving stale translation until the next gesture.
    public func reconcileScrollConfiguration() {
        guard let cellHeight = cellDimension?.height, cellHeight > 0 else { return }
        guard !terminal.isDisplayBufferAlternate else {
            viewportContentTranslationY = 0
            scrollAccumulator = 0
        ...
        position = min(
            baseMaximum + bottomOverscrollLimit(cellHeight: cellHeight, maximumRow: maxRow),
            max(-topOverscrollLimit(cellHeight: cellHeight), position)
        )
        applyVirtualScrollPosition(position, cellHeight: cellHeight, maximumRow: maxRow)
    }

    private func applyVirtualScrollPosition(
        _ position: CGFloat,
        cellHeight: CGFloat,
        maximumRow: Int
    ) {
        let baseMaximum = CGFloat(maximumRow) * cellHeight
        if position <= 0 {
            scrollTo(row: 0)
            viewportContentTranslationY = position
        } else if position >= baseMaximum {
            scrollTo(row: maximumRow)
            viewportContentTranslationY = position - baseMaximum
        } else {
            let row = max(0, min(maximumRow, Int(floor(position / cellHeight))))
            scrollTo(row: row)
            viewportContentTranslationY = position - CGFloat(row) * cellHeight
        }
    }

    private func bottomOverscrollLimit(cellHeight: CGFloat, maximumRow: Int) -> CGFloat {
        guard scrollPastLastLineMode != .disabled else { return 0 }
        let buffer = terminal.displayBuffer
        let cursorRow = max(0, min(buffer.lines.count - 1, buffer.yBase + buffer.y))
        let lastContentRow = (0..<buffer.lines.count).last { buffer.lines[$0].hasAnyContent() }
            ?? cursorRow
        let sourceRow: Int
        let targetRow: Int
        switch scrollPastLastLineMode {
        case .disabled:
            return 0
        case .lastLineWithContent:
            sourceRow = lastContentRow
            targetRow = 0
        case .lastLineInMiddle:
            sourceRow = lastContentRow
            targetRow = max(0, buffer.rows / 2)
        case .cursorLine:
            sourceRow = cursorRow
            targetRow = 0
        }
        let visibleSource = max(0, min(buffer.rows - 1, sourceRow - maximumRow))
        return CGFloat(max(0, visibleSource - targetRow)) * cellHeight
    }

    private func topOverscrollLimit(cellHeight: CGFloat) -> CGFloat {
        let effectiveMode: ScrollPastFirstLineMode
        switch scrollPastFirstLineMode {
        case .sameAsLastLine:
            switch scrollPastLastLineMode {
            case .disabled: effectiveMode = .disabled
            case .lastLineInMiddle: effectiveMode = .firstLineInMiddle
            case .lastLineWithContent, .cursorLine: effectiveMode = .firstLineWithContent
            }
        default:
            effectiveMode = scrollPastFirstLineMode
        }
        guard effectiveMode != .disabled else { return 0 }
        let buffer = terminal.displayBuffer
        let firstContentRow = (0..<buffer.lines.count).first { buffer.lines[$0].hasAnyContent() }
            ?? 0
        let targetRow: Int
        switch effectiveMode {
        case .disabled, .sameAsLastLine:
            return 0
        case .firstLineWithContent:
            targetRow = max(0, buffer.rows - 1)
        case .firstLineInMiddle:
            targetRow = max(0, buffer.rows / 2)
        }
    ...
    open override func scrollWheel(with event: NSEvent) {
        ...
        let gestureEnded = Self.scrollGestureEnded(
            phase: event.phase,
            momentumPhase: event.momentumPhase
        )
        if event.scrollingDeltaY == 0 {
            if gestureEnded, event.hasPreciseScrollingDeltas {
                scrollViewport(deltaY: 0, precise: true, gestureEnded: true)
            }
            return
        }
        guard let cellHeight = cellDimension?.height, cellHeight > 0 else { return }

        let reportsMouseWheel =
            allowMouseReporting
            && !nativeSelectionBypassesMouseReporting(for: event)
            && terminal.mouseMode != .off
        if !terminal.isDisplayBufferAlternate && !reportsMouseWheel {
            scrollViewport(
    ...
    /// Momentum events have their own lifecycle. Snapping at the finger-up event while momentum
    /// is beginning causes a visible hitch, while ignoring momentum's final event leaves a
    /// fractional row behind. Treat cancellation as an end in either lifecycle.
    static func scrollGestureEnded(
        phase: NSEvent.Phase,
        ...
        return touchEnded && momentumPhase.isEmpty
    }
