...
/// `Bundle.module` accessor. For executable targets that accessor does not
/// inspect a packaged macOS app's `Contents/Resources` directory and falls
/// back to an absolute path on the build machine, where absence is fatal.
/// Missing resources must make `Highlighter.init` return `nil`, not terminate
/// the host application.
enum HighlighterResourceBundle {
    static let name = "Highlighter_Highlighter.bundle"
    ...
    static func candidateURLs(
        ...
        var candidates = [
            mainResourceURL?.appendingPathComponent(name, isDirectory: true),
            mainBundleURL.appendingPathComponent(name, isDirectory: true),
    ...
    /// SwiftPM passes the real `.xctest` path via this structured flag. Do not
    /// treat arbitrary application arguments as resource search roots.
...
#if SWIFT_PACKAGE
        guard let bundle = HighlighterResourceBundle.locate() else {
            return nil
        }
#else
