  ...
  const sections = [
    ...
    { id: "agents", title: "智能体", description: "连接 Agent，显示状态并恢复原生会话。", special: "agents", groups: [
      { title: "智能体行为", rows: [
        ...
        row("agents.usageBarEnabled", "Pane 底部显示用量条", "Claude Code 运行时显示 5 小时 / 每周配额（用 Claude Code 的登录凭据读官方 /usage，与 Claude 的 /usage 面板同值；首次会请求一次钥匙串访问授权），Codex 另含当前会话上下文占比；关闭只隐藏，不改动 Agent 配置"),
