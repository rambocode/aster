...
PiP 从现有 `TerminalSession` 的 Ghostty GPU 帧构建系统 AVKit 镜像；原终端容器始终留在工作区，不再登记展示所有权或绘制占位。固定模式绑定 Pane UUID，跟随模式切换帧源并清空旧帧。独立透明源窗口负责系统 PiP 的视频图层与尺寸兼容，不影响原终端输入、网格或滚动状态。详见 [系统画中画](picture-in-picture.md)。
