...
PiP 从现有 `TerminalSession` 的 Ghostty GPU 帧构建系统 AVKit 镜像；原终端容器始终留在工作区，不再登记展示所有权或绘制占位。固定模式绑定 Pane UUID，跟随模式切换帧源并清空旧帧。独立透明源窗口负责系统 PiP 的视频图层与尺寸兼容，不影响原终端输入、网格或滚动状态。设置项 `pictureInPicture.style = interactive` 时，固定模式改由 `PaneFloatingTerminalController` 把终端 Host 借进置顶 `NSPanel`：这是唯一登记展示所有权的路径，`AppModel.floatingPaneID` 让工作区改画占位，关闭时归还。详见 [系统画中画](picture-in-picture.md)。

**横向标签栏（顶部/底部布局）**：满宽 `ThemeVisualEffectView` 材质条（tint 走 `tabbar.background` → `horizontalTabBarBackground` → `sidebarBackground` 回退链），标签行高取 `tab-bar.height`（默认 40pt）。顶部布局在标签行上方叠一条 28pt 标题带：中央目录胶囊（`makeWorkspaceHeader`）与交通灯共处这一行（对齐 Otty），内容区不再重复渲染标题行（`makeTerminalWorkspace` 按 `tabBarLayout == .top && showsTabBar` 跳过）；标签行因此整体落在系统标题栏拖拽命中区之下，标签顶缘不会被窗口拖动吃掉点击，从窗口左缘 10pt 起排。没有可选中标签时标题带退化为等高占位，标签行不上浮。底部布局直接贴内容区底边，标题行保持在内容区顶部，1pt `tabbar.border` 分隔线朝向内容区。组合用的纵向 `NSStackView` 默认按固有宽度居中，标签栏、分隔线与内容区三者都必须显式绑定外层 stack 宽度，缺约束时整条栏会缩成内容宽度的一小段浮在窗口中间。标签是内容自适应的胶囊：`TabRowButton` 与侧栏行共用「整行命中 + 内缩底卡」结构（横向上下各内缩 6pt、圆角同样取 `max(主题 tab.radius, 8)`），标题单行尾部截断（≤190pt）。选中胶囊右端常显小号 `xmark`（`IconHoverButton`，悬停自带底色反馈）；未选中标签是纯文字，悬停只有 `hoverBackground` 反馈、不出现关闭按钮，关闭走选中后的 `×`、右键菜单或 ⌘W。右侧 16pt 附件槽只在「选中或有状态徽章」时占宽，纯文字标签收紧为 0，悬停不改变宽度、标签排布不抖动。状态徽章与纵向共用 `makeActivityAccessory` 分支和徽章等价键去重（running spinner 不因重复刷新重启动画）；横向 idle 槽保持空，不显示 shell 名。标签之后只保留「新建标签页」一颗 `IconHoverButton`；命令面板不在标签行占图标位，走快捷键与菜单入口。标题与徽章仍走 `refreshTitle` / `refreshActivityBadge` 局部通道，不重建标签栏。
...
**分隔条与 Pane 拖放**：`PersistedSplitView` 的 `dividerThickness` 是 6pt 的命中区，`drawDivider(in:)` 只画中间 1pt（悬停 2pt）——默认 `AsterTheme.divider`，仅在「窗口是 key 且指针压在命中区上」时换成 `NSColor.controlAccentColor`（系统强调色，默认蓝；分隔条是工作区结构控件，与把手一样不走终端主题）；命中区 `NSTrackingArea` 由两个子视图的间隙算出，并在 `splitViewDidResizeSubviews` 里重建（自身 frame 不变时 AppKit 不会重建它）。移除感应区不会补发 `mouseExited`，指针恰好停在旧感应区里时高亮会卡住，因此每次重建后用 `mouseLocationOutsideOfEventStream` 对齐一次悬停状态。比例一律针对「扣掉分隔条的可用长度」，否则第一块会固定多出一个分隔条厚度。双击分隔条恢复等分。
...
标签拖入 Pane 复用同一套落点几何和覆盖层。`TabRowButton.mouseDown` 用 `trackEvents` 接管到抬起：没拖动才在抬起时派发选择（按下就选中会让被拖的标签自己变成当前标签），拖过 5pt 后经 `onDragMove` / `onDragEnd` 上报屏幕坐标。`WorkspaceViewController.finishTabDrag` 决定落点：本窗口的 Pane → `AppModel.mergeTab`；本窗口其它位置 → 只选中；窗口之外 → 原有的 `AsterAppDelegate.moveTab`。标签之间没有交换语义，中心区按默认方向并到右侧。

`mergeTab` 搬的是运行态对象而不是快照：源标签经 `detachAllPanesForMerge` 交出布局子树、`WorkspacePaneRuntime`、每 Pane 标题状态与 Agent 会话标题，目标标签经 `receivePanes` 用 `PaneLayout.inserting` 接到指定一侧。为此 `TerminalTabItem` 对会话的订阅按 Pane 分桶（`paneCancellables`），接线集中在 `attach(_:)`：会话回调整体覆盖写入，源标签交出时整桶释放，不会再收到已经不属于它的事件；受管终端的异步绑定结果经 `WorkspacePaneRuntime.onManagedTerminalBound` 转发给**当前**持有者。源标签不进「最近关闭」；它的某个 Pane 正在可交互画中画里时先关小窗。远端机器标签（结构归服务端）与当前选中标签由 `canMergeTab` 拦截；失败时载荷经 `restoreDetachedPanes` 还给源标签。

反方向是把 Pane 拖成独立标签：`beginPaneDrag` 的事件循环里多一类落点 `tabBarDropRect`（当前渲染的 `tabBarView`，即左侧栏或横向标签条；收起时为 nil），命中时整块高亮，松手走 `AppModel.movePaneToNewTab`。`TerminalTabItem.detachPaneForTransfer` 摘下单个 Pane 并交出同一种 `PaneMergePayload`，`closePane` 也复用它（关闭 = 摘下后 `stop()`），因此焦点归属、缩放清理和订阅释放只有一份实现。载荷里的描述符取自布局而不是 runtime：目录变化与受管终端引用只写回布局。新标签由 `TerminalTabItem(adopting:)` 建立——指定构造器的 `adoptedPanes` 走 `adopt` 接线而不是 `rebuildRuntimes`，否则会给同一个 Pane 再起一份 Shell；标题状态沿用该 Pane 的程序标题与目录回退，固定名/前缀重置为自动。新标签经 `insertTab(select: false)` 插在当前标签之后，当前标签保持选中。

两个方向都会让 Pane 换标签，`AsterControlBridge` 因此不能把「Pane 从某个标签的布局里消失」等同于关闭：`syncTabs` / `syncPanes` 在注销前用 `isPaneAlive` 确认它不再属于任何已接入窗口的标签；搬来的 Pane（`paneCache` 已有记录）沿用短 ID、改订到新标签并发 `pane.updated`，不重复宣告 `paneCreated`。
...
主窗口使用配置中的初始尺寸。`windowDidEndLiveResize` 只在拖动结束后保存新的内容尺寸，避免 live resize 期间触发工作区重建；独立设置窗口中的“恢复默认尺寸”可把主窗口恢复为 `1180 × 760 pt`。**工作区窗口不设 `setFrameAutosaveName`**：窗口尺寸的权威来源是 `appearance.windowWidth/Height`，AppKit 的 frame autosave 会成为第二份可写副本，而且它和 zoom 抢同一份「保存的 frame」——用户手动拖过窗口大小之后，双击标题栏放大会立刻被判定为「已放大」而缩回原尺寸，表现就是全屏一下又弹回去。回归覆盖在 `Tests/AsterTests/WorkspaceWindowGeometryTests.swift`。
