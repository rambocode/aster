...
`SettingsViewController` 使用 `NSSearchField`、`NSPopUpButton`、`NSSwitch`、`NSSlider`、`NSColorWell` 和 `NSGridView`，由唯一 `AsterSettingsWindowController` 承载。设置窗口独立于工作区，打开后主窗口及 Pane / PTY 保持原尺寸、原层级和完整可见状态；设置页不提供返回按钮，红灯只关闭设置窗口。窗口宽度固定 `700 pt`（min/max frame 宽度相同即锁死横向），高度可拉伸：下界 `460 pt`、上界不限，保留原生红绿灯位置，禁用 miniaturize，纵向 zoom 可用。**`700 pt` 宽度与 `460 pt` 最小高度是不可随分类、内容量或布局改动的产品兼容性约束；新增内容必须通过既有滚动区域承载。**高度跨启动记忆：`SettingsWindowGeometry`（AsterCore）给出宽度/下界/默认高度与 `clampHeight` 纯函数，`AsterSettingsWindowController` 用 `aster.settings.window-height.v1` 读写，delegate 在 `windowDidEndLiveResize` 与 `windowWillClose` 各记一次（逐帧写 UserDefaults 无意义，而纵向 zoom 不触发 live resize）。恢复时按 `NSScreen.main` 可视高度钳制——换到更小的屏幕后，超高的窗口会把标题栏推到菜单栏之上而无法拖动；拿不到屏幕信息时不钳制，宁可保留记忆值也不凭猜测缩窗口。窗口高度必须在 `contentViewController` 赋值**之后**套用：赋值会把窗口收缩回控制器视图的默认尺寸，写在前面的 `contentRect` 会被直接抹掉。该高度属于窗口状态，不进 `AsterConfiguration`，导出配置不会带走它。主题网格使用 `NSStackView.fillEqually` 四列，卡片宽度随可用空间等分（最小 84pt），长主题名在卡片内换行两行显示。卡片自身铺 5% ink 灰底（选中 9%、悬停 8%，并切手型指针）——缩略图多为浅色，没有底色时卡片会与网格容器糊成一片，看不出一张张卡的边界。
...
`AppKitMigrationTests` 静态确认主工作区和设置页不包含 SwiftUI Hosting，并检查 `NSSplitView`、九类设置、Glass 原生材质、整宽侧栏行、标签整理菜单、分组/排序/分隔线行为、28pt 标题区与设置页顶部锚定。`SettingsResponsivenessTests` 锁定设置窗口的宽度锁死 / 高度记忆与屏幕钳制，以及三条局部刷新不变量：切换分类后侧栏按钮与搜索框仍是同一批实例、搜索不重建内容区、开关与步进器改配置后内容区文档视图不变；`SettingsWindowGeometryTests`（AsterCoreTests）覆盖 `clampHeight` 的下界、屏幕上界与非法值。`WorkspacePanelLayoutTests`、`WorkspacePanelLayoutStoreTests` 与 `WorkspacePanelSplitViewTests` 覆盖纯宽度策略、窗口隔离、活动设置绑定、语义 divider、动态显隐和视图身份。完整测试还覆盖 24 套主题真值、终端、Recipe、文件安全与进程生命周期。发布前运行：
