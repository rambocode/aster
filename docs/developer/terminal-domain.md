...
`TerminalSession` 或任一工作区页面。调度参考 Ghostty revision
[`f64f4aca`](https://github.com/ghostty-org/ghostty/blob/f64f4aca2c29b554d111b36c3d946a9bddd159ff/src/termio/Exec.zig)
的 macOS gather/parse 策略：低于 1 KiB 的交互输出不增加固定等待；达到 1 KiB 的饱和流
最多合并 3 ms，并以 64 KiB 作为单次主线程 VT 解析上限。已经形成 backlog 后不再逐批
固定 sleep，而是在后续 RunLoop idle 继续排空。字节只在默认模式的 `beforeWaiting`
阶段消费，并且每轮最多一个批次；AppKit 输入源和已经排队的主线程界面任务因此先完成，
按钮跟踪等嵌套交互模式则完全不消费终端字节。Inspector 继续使用原生事件链，不订阅、
不窥视，也不等待终端消息总线。
...
flowchart LR
  ...
  B -->|interactive now / bulk 3 ms| G[up to 64 KiB ordered batch]
  G --> M[Main-thread terminal grid]
  M --> R[one pending display per frame]
