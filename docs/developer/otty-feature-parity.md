...
- 完成：[Frequent Folders](https://docs.otty.sh/workflows/frequent-folders) — OSC 7 自动学习、粘性忽略、时间衰减、100 项容量、Open Quickly 和 CLI `jump/learn/ignore` 已闭环。`learn` 与 `ignore` 各有三种一一对应的目标（目录 / 固定命令 / PATH 上的二进制），对齐 Otty 文档里 "ignore is the undo of learn" 的定义：撤销固定命令时保留真实使用历史（用户只是不想再把它钉在最前面），内置 Fig 规格不可删（它不是学出来的，删了无法恢复）。
...
- 完成：[Autocomplete / Inline Suggest](https://docs.otty.sh/terminal-features/autocomplete) — 输入停顿触发、inline ghost、四种接受方案及 Escape/Option-Escape/F5/自动候选面板均已接入；面板支持上下选择、Return/Tab/点击接受、8 行可见上限与超出后的滚动视窗、选中行整行淡染加前导标记条，以及显示选中项完整描述的右侧栏（窄 Pane 下降级回行内描述）。面板打开后继续输入会按新前缀收窄而不是关闭；预览和说明跟随当前选中项；Escape 关闭后本轮 prompt 内不再自动弹回；自动面板初始不选择首行，唯一候选或用户明确选择后才展示行内预览；空提示符推荐目录历史、固定命令、README 与失败纠错。inline 与面板同时关闭时完全不查询规格库。Shell 端 autosuggestion 已可见时，接受键直接发送其后缀，避免触发 Shell 原生补全询问；唯一候选仅显示 inline，手动面板收窄为一条也会收起。help 探测按「最深的不完整层级」触发并与内置规格按字段合并，描述折行不会变成假候选（Otty 在这一点上有解析 bug，其 docker 面板里能看到 `"warn",`）。候选来源按统一 relevance 排序并跨来源去重，参数槽位会从磁盘读取 git 分支/标签/remote、npm script、Makefile target 与 Homebrew formula，全程不执行被补全的工具。候选覆盖命令、子命令、选项、参数、文件、目录、Shell alias、固定命令、历史、README 与纠错。Inline ghost 会等 PTY 回显完成并更新光标位置后才显示，避免本地输入 tracker 领先 Shell 时与命令文字重叠；接受键与渲染共用同一可见性判定，没有任何可见建议时，多候选可打开面板；无候选时才放行给 Shell。Bundle 内置由 `scripts/build-fig-specs.mjs` 从 `@withfig/autocomplete` npm 包生成的 714 条完整 Fig 规格（嵌套子命令、选项、参数、静态候选与文件模板，schema v2，约 11.6 MB）；没有内置结构的命令首次输入参数时，在禁网且禁止文件写入的沙箱中按 `--help`/`-h`/`help` 生成独立本地规格。学习按目录、会话、频率、时间与固定次数排序，过滤 secret、glob 忽略、127 和错误长选项；`aster learn` 使用 0600 随机 token 鉴权。关闭本机学习会同时停止历史、README、help 探测与纠错，内置规格和文件补全仍可用。规格更新只能由设置页手动触发，从 Aster 仓库 raw 地址拉取同一份生成文件，且不覆盖本地规格；设置页状态与 Otty 对齐为 `v<上游日期> · N 条命令`。
...
Autocomplete 由 `PromptInputTracker`、`AutocompleteEngine`、`AutocompleteRelevance`、`AutocompleteLearningDatabase`、`AutocompleteDiskDynamicReader`、`AutocompleteService` 与 `TerminalAutocompleteController` 分层。OSC 133 只确定 prompt/command 生命周期，命令正文只从用户输入字节重建；Up/Down 等无法重建的 Shell 历史操作会停用当前 prompt 候选。学习、Fig 更新、本地 help 规格使用独立有界文件，手动更新不会覆盖本地学习。Shell alias 通过 OSC 6973 只发送有界名称，不发送展开正文。

排序是跨来源可比的单一 relevance 模型（`AutocompleteRelevance`），不再是分层加法常量：基础权重 + 匹配质量 + 归一化 frecency（7 天半衰期）+ 会话/pin 加成。历史条目的基础权重刻意低于子命令——历史赢下来必须靠 frecency，而不是靠“它是一条历史”。候选按「补全后的完整命令行」跨来源去重，因此历史整行与动态 token 候选会合并成一行。每个候选自带 `AutocompleteReplacementSpan`，ghost 显示与接受时插入的字节来自同一次计算。

动态候选（`AutocompleteDynamicSource`）把 Fig 规格里的生成器脚本映射成「读磁盘的意图」，只支持能从已有文件读出的来源（git refs/config/reflog、package.json、Makefile、Homebrew 的 Formula/Cellar/Caskroom 与 API 名称缓存）；需要执行工具才能取到的一律映射为 nil。`autocompleteDynamicReaderNeverSpawnsProcesses` 以源码文本断言守住「绝不运行被补全的工具」这条约束。按目录缓存以见证文件的 (mtime, size) 加硬 TTL 失效——目录 mtime 不随子目录内文件变化更新，单靠它会漏更新。

候选面板的可见性由显式状态机 `AutocompletePanelState` 承载，迁移只发生在 `refreshNow` 一处。打字保留面板状态（Otty 的 "keep typing to refine"），Escape 在本轮 prompt 内上闩锁，`auto` 模式下 Escape 兼任重新打开。自动弹出的面板在用户按方向键之前不武装 Return，避免系统主动弹出的浮层吞掉终端里最常用的按键。
...
参见 [自动补全领域](autocomplete.md)。Ghostty 的输入校验读取 `bufferInfo.cursor` 对应行，
分别核对光标前的完整输入和光标后的空白。每次 PTY 输出都先撤下 ghost，再在主线程重查；
接受行内建议时也使用相同校验，不能依赖一次性的“回显完成”标记。IME 的 x 是单元格中点，
补全 adapter 减去半格宽度；IME 候选窗口自身仍使用原始几何。
