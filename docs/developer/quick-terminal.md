...
- Shell 按用户意图退出（`exit` / Ctrl+D，即 `TerminalProcessTermination.closesPaneAutomatically`
  成立）时收起窗口并丢弃会话，下次呼出重新起一个干净的 Shell；与工作区 Pane 自动关闭同源，
  都经 `TerminalSession.onRequestCloseAfterExit`。丢弃延后一轮事件循环执行（回调发自会话自己的
  退出处理，同步销毁 surface 会在它收尾前释放内存），并等淡出动画结束，避免露出空白窗口。
- 信号终止、I/O 异常以及启动即非零退出保留最后画面，窗口菜单提供显式重启；运行中不能重启。
- 窗口与会话的生命周期分开：窗口在会话被丢弃后保留（淡出动画、位置与手动尺寸记忆都要用它），
  `show()` 只补建缺的那一半。
...
flowchart LR
  ...
  E -->|失焦且启用自动隐藏| F
  F --> A
  E -->|Shell 按用户意图退出| H[淡出后丢弃会话，窗口保留]
  H --> A
  E -->|退出应用| G[确认后销毁会话和热键]
```
...
的夹紧与失效规则、不可移动、失焦策略、会话身份、Shell 退出后收起窗口并重起新会话、
快速显隐与幂等清理；全量验证使用
