...
10. 写入 Agent 输入框的编码由目标决定，不由调用方强制：Ghostty 路径经 `GhosttySurfaceView.pasteText` 使用引擎协商模式，SwiftTerm 兼容路径的 `AsterTerminalView.typePromptText` 读取 `terminal.bracketedPasteMode`，协商过就按粘贴块投递，未协商才发裸 UTF-8。Claude Code / Codex 的输入框协商 2004 后会把裸字节流按逐键解释，`/`、`@`、方向键和候选列表会吃掉内容，表现为“点了发送但输入框没变化”。Prompt Queue 的 Return 在文本之后延时 60ms 单独发送 —— 同批到达的 CR 会被算进粘贴内容变成换行，只换行不提交。
...
12. Agent 会话标题投影：Pane 经 OSC 6974 精确绑定 provider + session ID 后，`AppModel.refreshAgentSessionTitles` 从已扫描历史匹配会话标题（清洗后仍等于文件名视为无标题），投影到标签行展示名与标题栏胶囊；用户固定标签名优先级更高。禁止目录/最近历史等松散匹配 —— 会把别的会话标题标到当前 Pane。标题是运行态，不进快照。Claude / Codex 的标题在首条 prompt 之后几秒才写盘，全量历史扫描（上千个文件）不能按这个节奏跑，因此运行中的会话由 `AgentSessionTitleTracker` 按单个会话探测：状态翻到 processing（prompt 刚提交）时按 2/3/5/10/20/40 秒递增重试，读到即停；绑定变化和其它状态翻转只读一次（接住 `/rename` 与恢复的旧会话）；没有 prompt 的空闲会话不产生事件，不读盘。`AgentSessionTitleProbe` 在后台任务里按 session ID 直接定位 `~/.claude/projects/<目录编码>/<id>.jsonl`（Pane 已 `cd` 走时遍历一级目录找同名文件；session ID 限定 `[A-Za-z0-9._-]` 防目录穿越；SSH 里的 Agent 不探测），单次最多读文件头 256KB + 尾 256KB，结果按发起时的 provider + session ID 回写，过期结果丢弃。探测到的标题优先于历史里的标题；重试用尽且该 Pane 仍无任何标题时才兜底重扫一次历史。其它 provider 未命中时仍按 5 秒节流触发历史重扫。更新走 `titleChanged` 局部通道，不进 `objectWillChange`。
13. Agent provider 识别与纠正：证据按强弱分三级。精确证据是 hook / 远端上报（Agent 进程自己报的身份）、命令首词（`AgentProvider.detect(executablePath:)`）和前台进程 argv；弱证据只有程序标题。首词识别不了时（alias、wrapper、没有 shell integration 的轮询兜底），`TerminalSession` 在命令开始后 0.3/1/3 秒读前台进程组组长的 argv（`ProcessArgumentsReader`，`sysctl(KERN_PROCARGS2)`），交给 `AgentProvider.detect(commandTokens:)` 按 `node <脚本路径>` 等运行时规则解包，与 tty7 的做法相同。标题补识别只认 Claude 的 `✳` / 半圆 spinner 前缀和别名前缀；盲文 spinner（U+2800–U+28FF）是多个 TUI 共用的忙碌动画——Codex 的标题就是 `⠙ <项目名>`——只剥掉、不当作 Claude 证据。标题得出的弱 provider 会被 hook、远端上报或 argv 改正，改正时一并作废按旧 provider 找到的 session ID 与定位任务；精确 provider 仍拒绝其它 provider 的上报。hook 没给 ID 时 `AgentSessionFileLocator` 按「项目目录 + 本次运行内修改」定位会话文件，并跳过其它存活 Pane（含其它窗口）已绑定的同 provider ID，同目录开两个 Agent 时不会绑到同一个会话、显示别人的标题。
...
14. 项目最近 Agent 会话作为补全首选项：`TerminalSession` 在 `.commandFinished` 与 PTY 退出两条路径、清空 Agent 绑定之前经 `onAgentSessionEnded` 上报 (provider, sessionID)，`TerminalTabItem` 附上 Pane 工作目录转给 `AppModel.recordEndedAgentSession`。`AgentProjectSessionRegistry`（AsterCore）以规范化绝对路径为键只保留每个目录最新一条、只收 `.resumeSession` 能力的 provider，经 `AgentProjectSessionStore` 存 defaults `aster.agent-project-sessions.v1`（解码限 1 MiB 并重新过滤）。展示与补全共用 `AppModel.projectAgentResumeCommand`（沿用用户自定义启动前缀，仅必要时加引号）。补全链路：`AppModel.projectAgentResumeSuggestion(for:)` → `TerminalTabItem.projectCommandSuggestionProvider` → `TerminalSession.projectCommandSuggestionProvider` → `TerminalAutocompleteController.projectCommandProvider`；控制器在 `refreshNow` 里按当前目录查询，命中且与已输入前缀匹配时以最高分整行候选（kind `.snippet`）插到首位（有纠错候选时排第二），`AutocompleteResult.make` 重算 ghost。**不自动执行**：这是刻意的，之前的「进入项目自动 resume」方案已被替换。结束时的 toast 时长按字数延长到最多 8s。
...
`AgentSetupService` 对所有目标先做祖先 symlink、文件类型、大小、格式和竞争变化检查，再原子写入；失败按相反顺序回滚。`Resources/agent-integration/aster-agent-hook.sh` 与生成的 plugin/extension 只上报生命周期，以及 provider 明确提供时由 ASCII 白名单和 128 字节上限约束的 `SessionID`；Codex/Claude command hook 的 stdin JSON 最多读取 256 KiB，只通过系统 `plutil` 提取顶层 `session_id`，prompt、tool 参数和输出不会进入 OSC。Grok Build 的 hooks 写入 `~/.grok/config.toml`（原生 `[[hooks.Event]]` 格式），不再与 Claude Code 共用 `~/.claude/settings.json`。Hook 脚本根据 `GROK_HOOK_EVENT` / `GROK_SESSION_ID` 判断调用方；Grok 的 session id 优先取注入的 `GROK_SESSION_ID`。OSC 6974 承载两类载荷，接收点先试 `TerminalBadgeDirective`、再 `AgentTerminalDirective`，两者键集合互斥、顺序无关，共用 256 字节上限。Claude Code 的 5h/7d 配额不经 hook、不经 statusLine：`ClaudeAccountQuotaService`（`Sources/Aster/ClaudeAccountQuotaService.swift`，进程单例，测试注入 fetch/token 闭包）用 Claude Code 自己维护的 OAuth token 直接 GET `https://api.anthropic.com/api/oauth/usage`（header `anthropic-beta: oauth-2025-04-20`），与 Claude 的 `/usage` 面板同源同值；token 通过 `/usr/bin/security find-generic-password -s "Claude Code-credentials" -w` 读登录钥匙串（回退 `~/.claude/.credentials.json`），只在内存缓存到 `expiresAt`，401/失败即丢弃。服务按 pane 引用计数：`TerminalSession.activeAgentProvider` 变为 `.claudeCode` 时 retain、离开时 release（只决定轮询频率，pane 自身不消费窗口数据）；有引用时每 60s 轮询，`agentTaskState` 从 processing 离开时 `refreshSoon`（2s 延迟、10s 最小间隔）。消费方是状态栏 AI 用量的 `UsageQuotaStore`（被动引用）。解析在 `ClaudeAccountQuotaParser`（AsterCore）：`five_hour|seven_day.utilization` 已是百分比，`resets_at` 是六位小数秒的 ISO 8601。**不用 statusLine 的原因**：它的百分比来自上一次 API 响应头，只在本 pane 收到响应时刷新，看不到其它 pane / 设备的消耗，且只有两位小数精度，常与 `/usage` 差 1%。Claude 因此没有「会话」上下文占比一格（接口不提供）。Codex 的配额随每次模型响应追加在 `~/.codex/sessions/**/rollout-*.jsonl` 的 `event_msg/token_count`（`rate_limits.primary/secondary{used_percent, window_minutes, resets_at}`，300=5h、10080=周，槽位与窗口无固定对应且可为 null，一律按 `window_minutes` 归类；会话占比 = `last_token_usage.total_tokens - reasoning_output_tokens` / `model_context_window`）。解析走 `CodexRolloutUsageParser`（读文件尾部 64 KiB），调用方是状态栏用量的 `CodexAccountQuotaReader`（一次性快照，只在 `codex app-server` 取不到额度时兜底）；per-pane 的 rollout 持续监听已随 Pane 底部用量条一起移除。

状态栏图标、用量浮动窗、token 统计与会话看板是独立的可选功能，规则与数据来源见 [AI 用量监控](ai-usage-monitor.md)；它复用本页的 `ClaudeAccountQuotaService`（被动引用）与控制桥的 Agent 状态投影，不另建配额服务或会话状态。

`AgentHistoryDiscoveryService` 有界读取已知 provider 路径：枚举阶段只收集元数据，按修改时间排序后再截到 500 个文件（先截后排会漏掉最新会话，标签行因此没有标题）；超过 4MB 的长会话只读文件头并按行截断，解析用 `overflow: .degrade`（超长记录跳过、记录数超限截尾），标题与项目目录都在文件头，不会因一条大 tool_result 丢掉整个会话；标题优先取 provider 自维护的会话名（`AgentSessionTitleIndex`：Claude 扫文件头 + 尾 256KB 里的 `custom-title` > `ai-title` 记录，只对含标记的行做 JSON 解码；Codex 读 `~/.codex/session_index.jsonl` 的 `thread_name`，rollout 本身没有标题记录），没有时才回落到首条用户消息（`AgentSessionTitleCleaner`）；Resume/Fork 由 `AgentSessionCommandPlanner` 保留 provider、model 和 system prompt 元数据。`FocusedAgentSessionContext` 冻结菜单动作所需的 provider、session ID 和 CWD，菜单载荷同时保留展开菜单时的工作区模型，防止菜单跟踪期间 key window 变化后把 Fork 路由到其它窗口；`AgentContinuationPlacement` 把 Fork 明确路由到当前新窗口 Pane、新标签或指定方向分屏，新 PTY 就绪后才发送 provider 原生命令。
...
- Claude 配额：钥匙串读取被拒、无 token、接口非 200 或响应缺窗口时不出数、不弹错误，保留上一份数据直到下次成功。`TerminalSession` 只在 provider 是 Claude 时 `retain()` 服务以提高轮询频率，自身不消费窗口数据。
