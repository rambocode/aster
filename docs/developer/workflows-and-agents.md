...
15. Pane 底部 Agent 用量条只展示、不做任何自动行为（不阻止发送、不通知、不参与派发）。数据由 `TerminalSession.agentUsage` 发布，**不进 `TerminalTabItem.objectWillChange`**（轮询和 rollout 追加会高频重发，走聚合会按该频率重建工作区）；`WorkspaceView` 按 Pane 订阅并对 `AgentUsageBarView` 原地 `apply`，`AgentUsageSnapshot` 的 `==` 刻意忽略 `updatedAt` 以抑制同值重发。条挂在 `ActivePaneHostView.setStatusStrip`（底部第二槽，Prompt Queue 的 `setBottomAccessory` 在其上，内容底边跟随最上面的槽，约束链整体重建），高度 20pt 显式常量。
...
- 用量条：rollout 找不到、被删除或改名时条消失而不报错；Claude 的钥匙串读取被拒、无 token、接口非 200 或响应缺两个窗口时不显示条、不弹错误，保留上一份数据直到下次成功。
