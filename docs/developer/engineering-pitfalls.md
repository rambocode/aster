...
`TerminalLaunchEnvironmentTests`(runloop 泵探针)、`TerminalOutputStormTests`(重建风暴与滚动语义)、`RestoredWorkspaceInputProbeTests`(恢复输入 + 点击自愈 + 退出恢复)、`InteractiveSplitGeometryProbeTests`(分屏几何与褪色)、`NestedSplitGeometryProbeTests`(嵌套分屏收敛定位)、`SplitPaneFocusProbeTests`(拆分后键盘焦点跟随)、`SplitPaneTests`(焦点褪色局部翻转)、`PrivateUseGlyphFallbackTests`(PUA 字形 cascade 解析)、`CursorAlignmentProbeTests`(光标网格对齐)、`MetalCaretInvariantProbeTests`(Metal 激活时 AppKit caret 隐藏)。已知:全量 `--no-parallel` 会在 AppKitMigrationTests 中途静默 exit(0)(2026-08-16 观测:约 515/951 处截断,`atexit` 侦探抓到退出栈为 `swift_task_asyncMainDrainQueue → exit(0)`——Swift Concurrency 把测试 main task 判为「已完成」而正常退出;止点随 skip 平移,是套件累积效应而非单一测试,HEAD 基线即可复现)。两个后果:一,exit 0 让截断看起来像「全绿」,必须核对 `Test run with N tests` 总结行;二,止点之后存在**从未被执行的阴影区**——已发现 `togglingInspectorDoesNotRebuildWorkspaceViews`、`workspaceViewControllerUsesPanelLayoutForAllThreeColumns`、`splitPanesFillContentArea` 等 6 个测试早已腐烂(仍期望迁移前的 `AsterTerminalView`,单独串行跑必红)。当前验证方案:并行全量(核对总结行,忽略 PTY 并发敏感套件的预期失败)+ 对 PTY 敏感套件与本次改动相关测试定向 `--no-parallel --filter` 补跑;PTY 套件不要与创建 Ghostty 视图的 UI 测试混在同一次 filter 里(互相污染)。
